...
class DecisionTreeModel:
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series):
        # convert DataFrame to numpy array, call the _fit method
        X_array = X.to_numpy()
        Y_array = y.to_numpy()
        print("Start fitting")
    ...
    def _build_tree(self, X, y, depth=0):
        ...
        # Stop recursively building the tree(split the node) when hits stopping criteria
...
# def _test():
#     df = pd.read_csv('archive/tennis_dataset/tennis.csv')
#     X = df.drop(['Label'], axis=1)
#     y = df['Label'].apply(lambda x: 0 if x == '+' else 1)
#
#     df = pd.read_csv('archive/tennis_dataset/tennis_test.csv')
